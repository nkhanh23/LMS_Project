    ...
    <form class="row g-3" method="post" action="{{ route('admin.site-setting.store') }}">
        ...
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" name="address" id="address" placeholder="Enter the address"
                value="{{ old('address', $site_data->address ?? '') }}">
        </div>

        <div class="col-md-6">
            <label for="phone" class="form-label">Số điện thoại</label>
            <input type="text" class="form-control" name="phone" id="phone" placeholder="Enter the course name"
                value="{{ old('phone', $site_data->phone ?? '') }}">
        </div>
